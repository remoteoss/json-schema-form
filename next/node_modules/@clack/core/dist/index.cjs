...
`).length-1;this.output.write(sisteransi.cursor.move(-999,t*-1))}render(){const t=wrapAnsi(this._render(this)??"",process.stdout.columns,{hard:!0});if(t!==this._prevFrame){if(this.state==="initial")this.output.write(sisteransi.cursor.hide);else{const F=diffLines(this._prevFrame,t);if(this.restoreCursor(),F&&F?.length===1){const E=F[0];this.output.write(sisteransi.cursor.move(0,E)),this.output.write(sisteransi.erase.lines(1));const h=t.split(`
`);this.output.write(h[E]),this._prevFrame=t,this.output.write(sisteransi.cursor.move(0,h.length-E-1));return}if(F&&F?.length>1){const E=F[0];this.output.write(sisteransi.cursor.move(0,E)),this.output.write(sisteransi.erase.down());const h=t.split(`
