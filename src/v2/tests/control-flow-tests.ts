...
export const oneOfTestCases = [
  {
    ...
    invalidTestCases: [
      { data: 3, error: { '': 'must match exactly one schema but matched 2 schemas' } },
      { data: 1.5, error: { '': 'must match exactly one schema but matched 0 schemas' } },
    ],
  },
  {
    ...
    validTestCases: [
      {
        data: 'foobar',
        description: 'one oneOf valid (matches minLength only)',
      },
    ],
    invalidTestCases: [
      {
        data: 3,
        error: {
          '': 'this must be a `string` type, but the final value was: `3`.',
        },
      },
      {
        data: 'foo',
        error: {
          '': 'must match exactly one schema but matched 2 schemas',
        },
      },
    ],
  },
  {
    ...
    validTestCases: [], // nothing can be valid when all schemas are true
    invalidTestCases: [
      {
        data: 'foo',
        description: 'any value is invalid because it matches all schemas',
        error: {
          '': 'must match exactly one schema but matched 3 schemas',
        },
      },
      {
        data: 123,
  ...
  },
  {
    ...
    invalidTestCases: [], // nothing can be invalid because exactly one schema (true) always matches
  },
];
